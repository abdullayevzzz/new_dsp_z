...
for p in ports:
    if 'XDS100' in p[1]:
        ...
        port_name = p[0]
        global ser
        ser = serial.Serial(port_name, 230400, timeout=1)
        break
...
buffer = ser.read(20)
i = 0
while not (buffer[i] == 0x7F and buffer[(i+1, 0)[i+1 == 20]] == 0xFF):
...
while not keyboard.is_pressed("s"):
    ...
    if (buffer[0] == 0x7F and buffer[1] == 0xFF):   #confirm synchronization
        ...
        if (j >= 150):
            j = 0
            axA.cla()
            axA.plot(t, list(BIOZ1_data.downsampled))
            axA.set_title('BIOZ 1', fontweight="bold")
                        
            axB.cla()
            axB.plot(t, list(BIOZ2_data.downsampled))
            axB.set_title('BIOZ 2', fontweight="bold")  
            
            axC.cla()
            # axC.plot(list(range(len(ECG_downsampled_queue.queue))), list(ECG_downsampled_queue.queue))
            axC.plot(t, list(ECG_data.downsampled))
            axC.set_title('ECG', fontweight="bold")

            #axC.cla()
            #axC.plot(t,ECG_aver_filt[0:fulLen//5-4])
            #axC.set_title('ECG_Filtered', fontweight ="bold")
        ...
        print('Synchronization not confirmed')
        i = 0
        while not (buffer[i] == 0x7F and buffer[(i+1, 0)[i+1 == 20]] == 0xFF): #modify code, make unique sync. code
            i = i+1    #find syncronization
        ser.read(i) #throw away until next syncronisation
