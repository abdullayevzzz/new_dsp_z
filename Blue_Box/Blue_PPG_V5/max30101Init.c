...
void initMAX30101(void)
{
    ...
    // test = readRegister8(MAX30105_PARTID);
    if (test == 0x15){
        int i;
        for (i = 0; i<5; i++){
            GPIO_writePin(34, 0);
...
void setup(){
    ...
    */
    ...
    writeRegister8(MAX30105_MODECONFIG, MAX30105_MODE_REDONLY);
    writeRegister8(MAX30105_PARTICLECONFIG, MAX30105_ADCRANGE_16384 | MAX30105_SAMPLERATE_3200 | MAX30105_PULSEWIDTH_118);
    writeRegister8(MAX30105_LED1_PULSEAMP, 0x7F);
...
void readRegister(uint8_t reg, uint8_t result[], uint8_t num){
    ...
    DEVICE_DELAY_US(100);
    I2C_setConfig(I2CB_BASE, (I2C_MASTER_RECEIVE_MODE));
    I2C_setDataCount(I2CB_BASE, num);
    I2C_sendStartCondition(I2CB_BASE);
    DEVICE_DELAY_US(100);
    int i;
    for (i=0; i<num; i++)
        result[i] = I2C_getData(I2CB_BASE);
}
...
void writeRegister8(uint8_t reg, uint8_t value){
    ...
    */
    ...
    // Set transmit and master bit
    HWREGH(I2CB_BASE + I2C_O_MDR) |= I2C_MDR_MST | I2C_MDR_TRX;

    // Send start - maybe together with trx and mst??
    HWREGH(I2CB_BASE + I2C_O_MDR) |= I2C_MDR_STT;
