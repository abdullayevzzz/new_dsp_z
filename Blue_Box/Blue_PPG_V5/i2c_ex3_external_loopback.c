...
void main(void)
{
    ...
    Device_initGPIO();
    // Initialize GPIOs 32 and 33 for use as SDA B and SCL B respectively
...
//    SCI_setConfig(SCIA_BASE, DEVICE_LSPCLK_FREQ, 230400,
   ...
   // uint32_t tempValues[10];
    ...
    if (numOfNewSamples() == 3){
        readRegister(MAX30105_FIFODATA, ppg_sample, 9);
        pack(txdata, ppg_sample);
        int j;
        for (j=0; j<8; j++) // OLev 3 -> 18
            HWREGH(SCIB_BASE + SCI_O_TXBUF) = txdata[j]; //send data
     }
    }
}
