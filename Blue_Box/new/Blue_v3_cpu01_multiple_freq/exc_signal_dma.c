...
void set_excitation_frequency(const uint16_t* activeFreqs, const SignalsStruct* signalsArray) {
    ...
    float offset;
    float waveformValue;
    sigGen(SINE_TBL_P,10,BUFLEN, 's');

    for(j=0;j<SINE_TBL_SIZE;j++)
        {
            SINE_TBL_P[j] = (int16_t)(SINE_TBL_P[j]*0.5 + 1000)*2.0475;  //  shift and scale for DAC
        }
...
// set_excitation_frequency - Sum and normalize all active frequencies
void set_excitation_frequency(const uint16_t* activeFreqs, const SignalsStruct* signalsArray) {
    float waveformGain = 0.5; // Range 0.0 -> 0.5
    uint16_t i, j;
    uint16_t active_count = 0;  // Counter for active frequencies

    // Clear the sine table before summing contributions
    for (j = 0; j < SINE_TBL_SIZE; j++) {
        SINE_TBL_P[j] = 0;
    }
    ...
    for (i = 0; i < 10; i++) {
        if (activeFreqs[i]) {  // If the frequency is active
            for (j = 0; j < SINE_TBL_SIZE; j++) {
                SINE_TBL_P[j] += signalsArray[i].RefSigSin[j];  // Sum the sine signals
            }
            active_count++;
        }
    }

    // Normalize by the number of active frequencies
    if (active_count > 0) {
        for (j = 0; j < SINE_TBL_SIZE; j++) {
            // SINE_TBL_P[j] = (int16_t)((SINE_TBL_P[j] / active_count) * waveformGain + 2047);
            // SINE_TBL_P[j] = SINE_TBL_P[j] * 2 * waveformGain / active_count + 2047;
            SINE_TBL_P[j] = SINE_TBL_P[j] * waveformGain / active_count + 2047;
            SINE_TBL_N[j] = 4095 - SINE_TBL_P[j];  // Complement for DAC
        }
    }

    // Update PWM compare values for one freq only
    for (i = 0; i < 10; i++){
        if (activeFreqs[i] == 1) // choose the first active frequency
            break;
    }
    initEPWM1(freqValues[i]);
}

void use_pwm_excitation(void){
    EALLOW;
    // Disable DAC
    DAC_PTR[1]->DACOUTEN.bit.DACOUTEN = 0;
    DAC_PTR[2]->DACOUTEN.bit.DACOUTEN = 0;
...
void use_dac_excitation(void){
    ...
    CpuSysRegs.PCLKCR2.bit.EPWM1=0;      // Module clock is gated-off
    GpioCtrlRegs.GPADIR.bit.GPIO0 = 0;   // Set GPIO as input
