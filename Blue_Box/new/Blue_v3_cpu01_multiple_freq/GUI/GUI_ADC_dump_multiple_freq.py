...
skip_frame = False
current_value = 0.0  # Example initial current value
fulLen = 1000 # Window length
downsample_rate = 1
# numOfFreqs = int((packet_size - 13)/8)
numOfFreqs = 10
my_baudrate = 460800
...
freq_dict = {'4 kHz': 0, '7 kHz': 1, '10 kHz': 2, '20 kHz': 3, '40 kHz': 4, '70 kHz': 5, '100 kHz': 6, '200 kHz': 7,
                     '400 kHz': 8, '800 kHz': 9}
...
class FrequencyController:
    def __init__(self, def_feq):
        self.dict = freq_dict
        self.anti_dict = {v: k for k, v in freq_dict.items()}
...
class WaveData:
    def __init__(self, size):
        ...
        self.downsampled = deque([0] * (self.size // downsample_rate), maxlen=self.size // downsample_rate)
        self.filter_50 = deque([0] * (20 // downsample_rate), maxlen=4)
        self.filter_100 = deque([0] * (10 // downsample_rate), maxlen=2)
        self.temp = []
        self.prev = 0
        self.prev_prev = 0
    ...
    def add(self, value):
        ...
        if len(self.temp) == downsample_rate:
            average = sum(self.temp) / downsample_rate
...
filter_50_mode = False
filter_100_mode = False
filter_median_mode = False

# Initialize FIFO queues
Z_mag_data = [WaveData(fulLen) for _ in range(10)]
Z_phase_data = [WaveData(fulLen) for _ in range(10)]
...
Excitation_raw = WaveData(fulLen)
Response_raw = WaveData(fulLen)
...
def select_exc_mode(label):
    if label == 'DAC':
        print('DAC mode selected')
        ser.write(b'e1d')
    elif label == 'PWM':
        print('PWM mode selected')
        ser.write(b'e1p')
    ser.read(packet_size * 250)
...
def create_wave_figure():
    ...
    t = list(range(0, fulLen, downsample_rate))
    lines = []
    for ax in axs:
    ...
    lines[-2], = axs[-2].plot(t, list(Excitation_raw.downsampled))
    axs[-2].set_title('EXCITATION_RAW', fontweight="bold")
    lines[-1], = axs[-1].plot(t, list(Response_raw.downsampled))
    axs[-1].set_title('RESPONSE_A_RAW', fontweight="bold")
...
def create_common_buttons():
    ...
    # Create an axis for text display (place it at a suitable location)
    text_ax = plt.axes([0.25, 0.15, 0.1, 0.1], frameon=False)  # Adjust position
    button_axs.append(text_ax)
    text_ax.set_xticks([])
    text_ax.set_yticks([])

    # Add text to display current value
    current_text = text_ax.text(0.5, 0.5, f"Current:\n{current_value:.2f}uA",
    ...
    # Add the text 'Excitation'
    current_label_ax = plt.axes([0.4, 0.22, 0.1, 0.03])  # Position for the label
    current_label_ax.axis("off")  # Turn off the axis for clean text
    current_label_ax.text(0.5, 0.5, "Excitation", fontsize=10, fontweight="bold", ha="center", va="center")
    # Add the 'Excitation' radio button
    button_axs.append(plt.axes([0.4, 0.15, 0.1, 0.07], facecolor='white'))  # Adjusted position
    radio_exc = RadioButtons(button_axs[-1], ('DAC', 'PWM'), active=0)
    radio_exc.on_clicked(select_exc_mode)  # Connect to callback function
    buttons.append(radio_exc)
...
def create_wave_buttons():
    ...
    buttons.append(CheckButtons(button_axs[-1], labels, (False, False, False)))
...
def try_connect_bluetooth(port_device, result):
    try:
        with serial.Serial(port_device, my_baudrate, timeout=0.5) as temp_ser:
...
def auto_find_port():
    ...
    for port in ports:
        if 'XDS100' in port.description or 'CP210x' in port.description:
...
if ser_emul:
    ...
    ser = serial.Serial(com_port_name, my_baudrate, timeout=1)
else:
...
while len(buffer) != packet_size:
    buffer = ser.read(packet_size)
    print(buffer, len(buffer), 'Communication problem')
i = 0
...
while True:
    ...
    if len(buffer) != packet_size:
        ...
        if mode == 'd':
            exc_raw = int.from_bytes(buffer[9:11], byteorder='little', signed="True")
            resp_raw = int.from_bytes(buffer[11:13], byteorder='little', signed="True")

            is_first_freq = True
            print_vals = ""
            for i in range(0, numOfFreqs):
                index_add = i*8
                accumA1I = int.from_bytes(buffer[13+index_add:15+index_add], byteorder='little', signed="True")
                accumA1Q = int.from_bytes(buffer[15+index_add:17+index_add], byteorder='little', signed="True")
                accumB1I = int.from_bytes(buffer[17+index_add:19+index_add], byteorder='little', signed="True")
                accumB1Q = int.from_bytes(buffer[19+index_add:21+index_add], byteorder='little', signed="True")
                ...
                print_vals += f"{round(abs(Z1), 4)}, "
                Z_mag_data[i].add(abs(Z1))
                Z_phase_data[i].add(cmath.phase(Z1))

                if is_first_freq:
                    is_first_freq = False
                    Z2 = 0
                    current_value += 0.0687 * abs(accumA1) / (signal_ranges.coef_float('EXC') * abs(refRes))
                    magZ1 = abs(Z1)
                    phZ1 = cmath.phase(Z1)
                    magZ2 = abs(Z2)
                    phZ2 = cmath.phase(Z2)
        ...
        Excitation_raw.add(exc_raw)
        Response_raw.add(resp_raw)
        if op_mode == 0:
            # print('Wave mode')
            if mux_mode_cur == 1:
    ...
    if logFlag or ref_update_flag:
        if op_mode in (0,):
            current_row = []
            for i in range(numOfFreqs):
                current_row.append(round(Z_mag_data[i].raw[-1], 6))
                current_row.append(round(Z_phase_data[i].raw[-1], 6))
            current_row.append(packet_number)
    ...
    circular_counter += 1
    if (circular_counter >= 1000) and op_mode in (0,):
        ...
        with warnings.catch_warnings():
            ...
            lines[-2].set_ydata(list(Excitation_raw.downsampled))
            axs[-2].set_ylim(min(Excitation_raw.downsampled), max(Excitation_raw.downsampled))
            lines[-1].set_ydata(list(Response_raw.downsampled))
            axs[-1].set_ylim(min(Response_raw.downsampled), max(Response_raw.downsampled))
            # print('Response PP: ', max(Response_raw.downsampled) - min(Response_raw.downsampled))
            fig.canvas.draw_idle()
            plt.pause(0.1)
