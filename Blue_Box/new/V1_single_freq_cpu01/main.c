    ...
    while(1){
        ...
        */
        ...
        else if ((buffer_status & 0b00100010) == 0b00100010) ////Check if Part 2 is filled and next expected is Part 2
        {
            ...
            /*
            accumA2I = _dmac(signal2sin,adcAResults,BUFLEN/40-1,0);
            accumA2Q = _dmac(signal2cos,adcAResults,BUFLEN/40-1,0);
            accumB2I = _dmac(signal2sin,adcBResults,BUFLEN/40-1,0);
            accumB2Q = _dmac(signal2cos,adcBResults,BUFLEN/40-1,0);
            accumC2I = _dmac(signal2sin,adcCResults,BUFLEN/40-1,0);
            accumC2Q = _dmac(signal2cos,adcCResults,BUFLEN/40-1,0);
            */
        ...
        else if ((buffer_status & 0b10001000) == 0b10001000) //Check if Part 4 is filled and next expected is Part 4
        {
            ...
            /*
            accumA2I += _dmac(signal2sin+BUFLEN/2,adcAResults+BUFLEN/2,BUFLEN/40-1,0);
            accumA2Q += _dmac(signal2cos+BUFLEN/2,adcAResults+BUFLEN/2,BUFLEN/40-1,0);
            accumB2I += _dmac(signal2sin+BUFLEN/2,adcBResults+BUFLEN/2,BUFLEN/40-1,0);
            accumB2Q += _dmac(signal2cos+BUFLEN/2,adcBResults+BUFLEN/2,BUFLEN/40-1,0);
            accumC2I += _dmac(signal2sin+BUFLEN/2,adcCResults+BUFLEN/2,BUFLEN/40-1,0);
            accumC2Q += _dmac(signal2cos+BUFLEN/2,adcCResults+BUFLEN/2,BUFLEN/40-1,0);
            */
            ...
            switch (received_command[0]){
                ...
                */
                case 'f':  // frequency signal
                    ...
                    uint16_t freq_kHz = freq_table[received_command[2] - '0'];  // convert to the digit and then fetch the frequency
                    sigGen(signal1sin, freq_kHz, BUFLEN, 's');
                    sigGen(signal1cos, freq_kHz, BUFLEN, 'c');
                    set_excitation_frequency(freq_kHz);  // frequency in kHz
                    mux_counter = 0;
                    break;
