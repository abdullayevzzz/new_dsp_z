...
# choose_wave_tomo('Tomo2')
ser.reset_input_buffer()
while True:
    old_packet_number = packet_number   # debug
    buffer = ser.read(packet_size)
    ...
    if len(buffer) != packet_size:
        ...
        dif = packet_number - old_packet_number
        if dif not in (1, -255):
            print('Missing packets ', dif)

        # print('Sync confirmed', time.time(), prev_time)
        if time.time() - prev_time > 5:
            prev_time = time.time()
            n = ser.in_waiting
            print(f'waiting bytes: {n}')
            print(dif)
            # ser.reset_input_buffer()
    ...
    circular_counter += 1
    if (circular_counter >= 1000) and op_mode == 0:
        circular_counter = 0
