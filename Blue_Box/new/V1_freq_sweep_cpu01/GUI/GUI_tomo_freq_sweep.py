...
while True:
    ...
    if logFlag or ref_update_flag:
        ...
        # elif (tomo_mode is True) and (tomo_mux_updated is True):  # Only log 1 averaged value per mux position
            ...
            freq_log_counter += 1
            if freq_log_counter == 192:
                if exc_pin == 16 and sns_ap_pin == 14 and len(row_buffer) == 192 and active_freq == next_freq_index:
                    ...
                    if next_freq_index == 10:
                        ...
                        exit()
                        quit()
                    # freq_log_counter = - 50  # this will throw away the first frame
                else:
                    print('aborted', freq_log_counter, exc_pin_index, sns_ap_index)
                    # raise ValueError('wroonnng')

            elif exc_pin == 16 and sns_ap_pin == 14:
                print('log synchro mismatch correction')
                freq_log_counter = 0  # this will throw away the first frame
                row_buffer.clear()

            if ref_update_flag:
                ref_count += 1
                if ref_count == 192:
                    if exc_pin_index == 15 and sns_ap_index == 13:
                        ref_update_flag = False
                        f.close()
