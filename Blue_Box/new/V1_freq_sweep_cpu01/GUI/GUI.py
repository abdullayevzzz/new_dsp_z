...
class EIT_reconstruct:
    ...
    def init_plot(self):
        ...
        if self.use_shape == 1:
            ...
            # self.mesh_obj = mesh.create(self.n_el, h0=0.05, fd=mesh.shape.thorax)
...
class TomoData:
    ...
    def average(self):
        if self.raw:
            self.filtered = median_filter(self.raw, size=self.med_fil_size, mode='nearest')
            average = sum(self.filtered) / len(self.filtered)
