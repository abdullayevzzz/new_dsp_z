...
int mux_inputs_3[] = {EXC_R, SNS_AP, SNS_AN, SNS_BP, SNS_BN, RET_GND};
int mux_outputs_3[] = {OUT_13, OUT_14, OUT_15, OUT_17, OUT_18, OUT_19}; // Outputs are in range 1-32
int num_pairs_3 = 6; // Enter the number of pairs

int mux_inputs_4[] = {EXC_R, SNS_AP, SNS_AN, SNS_BP, SNS_BN, RET_GND};
int mux_outputs_4[] = {OUT_19, OUT_20, OUT_21, OUT_23, OUT_24, OUT_25}; // Outputs are in range 1-32
int num_pairs_4 = 6; // Enter the number of pairs
...
void execute_tomography_mux(void) {
    ...
    static uint16_t num_pairs = 4; // Always four pairs
    static uint16_t* tomo_inputs = mux_inputs;  // Inputs are the same all the time

    if (mux_counter < mux_period)
        return;

    if (sns_ap_out == ret_out) {
        sns_ap_out = sns_ap_out + 1;
    }

    if (sns_ap_out < NUM_OUTPUTS) {
        uint16_t tomo_outputs[] = {mux_outputs[exc_out], mux_outputs[sns_ap_out], mux_outputs[ret_out], mux_outputs[ret_out]}; // RET_GND and SNS_AN share the same output
        mux_busy_flag = 1;
        router_config(num_pairs, tomo_inputs, tomo_outputs);
        mux_busy_flag = 0;
        mux_mode = (ret_out << 10) | (exc_out << 5) | (sns_ap_out);
        sns_ap_out++; // Move to next sns_ap_out
    }

    else {
        sns_ap_out = 0;
        exc_out++;
        if (exc_out >= NUM_OUTPUTS) {
            exc_out = ++ret_out + 1;
        }
    }

    if (ret_out >= NUM_OUTPUTS - 1) {
        ret_out = 0; // Reset ret_out for next series of multiplexing
        exc_out = ret_out + 1;
        sns_ap_out = 1; // Reset sns_ap_out to start over
