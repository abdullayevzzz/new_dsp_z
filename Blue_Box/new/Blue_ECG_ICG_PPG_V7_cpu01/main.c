...
uint16_t mux_period = 2; // mux period in ms
...
{   sigGen(signal1sin,10,BUFLEN, 's');
    ...
    while(1){
        ...
        {
            accumA1I = _dmac(signal1sin,adcAResults,BUFLEN/40-1,0);  //half of BUFLEN/10 (10 rpt calculates up to BUFLEN/2)
            accumA1Q = _dmac(signal1cos,adcAResults,BUFLEN/40-1,0);
            accumB1I = _dmac(signal1sin,adcBResults,BUFLEN/40-1,0);
            accumB1Q = _dmac(signal1cos,adcBResults,BUFLEN/40-1,0);
            accumC1I = _dmac(signal1sin,adcCResults,BUFLEN/40-1,0);
            accumC1Q = _dmac(signal1cos,adcCResults,BUFLEN/40-1,0);
            int j;
            for (j = NUM_OF_BYTES / 2; j < NUM_OF_BYTES; j++){
        ...
        else if (buffer_status == 0b110  ||  buffer_status == 0b111) //wait until full buffer to fill and next expected flag is 1
        {
            ...
            accumA1I += _dmac(signal1sin+BUFLEN/2,adcAResults+BUFLEN/2,BUFLEN/40-1,0);  //half of BUFLEN/10 (10 rpt calculates up to BUFLEN/2)
            accumA1Q += _dmac(signal1cos+BUFLEN/2,adcAResults+BUFLEN/2,BUFLEN/40-1,0);
            accumB1I += _dmac(signal1sin+BUFLEN/2,adcBResults+BUFLEN/2,BUFLEN/40-1,0);
            accumB1Q += _dmac(signal1cos+BUFLEN/2,adcBResults+BUFLEN/2,BUFLEN/40-1,0);
            accumC1I += _dmac(signal1sin+BUFLEN/2,adcCResults+BUFLEN/2,BUFLEN/40-1,0);
            accumC1Q += _dmac(signal1cos+BUFLEN/2,adcCResults+BUFLEN/2,BUFLEN/40-1,0);
        ...
        if(received_command){
            ...
            switch (received_command){
                ...
                case '0': // 1 kHz
                    ...
                    break;
                case '1': // 2 kHz
                    sigGen(signal1sin, 2, BUFLEN, 's');
                    sigGen(signal1cos, 2, BUFLEN, 'c');
                    set_excitation_frequency(2);  // frequency in kHz
                    mux_counter = 0;
                    break;
                case '2': // 4 kHz
                    sigGen(signal1sin, 4, BUFLEN, 's');
                    sigGen(signal1cos, 4, BUFLEN, 'c');
                    set_excitation_frequency(4);  // frequency in kHz
                    mux_counter = 0;
                    break;
                case '3': // 7 kHz
                    sigGen(signal1sin, 7, BUFLEN, 's');
                    sigGen(signal1cos, 7, BUFLEN, 'c');
                    set_excitation_frequency(7);  // frequency in kHz
                    mux_counter = 0;
                    break;
                case '4': // 10 kHz
                    sigGen(signal1sin, 10, BUFLEN, 's');
                    ...
                    break;
                case '5': // 20 kHz
                    sigGen(signal1sin, 20, BUFLEN, 's');
                    sigGen(signal1cos, 20, BUFLEN, 'c');
                    set_excitation_frequency(20);  // frequency in kHz
                    mux_counter = 0;
                    break;
                case '6': // 40 kHz
                    sigGen(signal1sin, 40, BUFLEN, 's');
                    sigGen(signal1cos, 40, BUFLEN, 'c');
                    set_excitation_frequency(40);  // frequency in kHz
                    mux_counter = 0;
                    break;
                case '7': // 70 kHz
                    sigGen(signal1sin, 70, BUFLEN, 's');
                    sigGen(signal1cos, 70, BUFLEN, 'c');
                    set_excitation_frequency(70);  // frequency in kHz
                    mux_counter = 0;
                    break;
                case '8': // 100 kHz
                    ...
                    break;
                case '9': // 200 kHz
                    sigGen(signal1sin, 200, BUFLEN, 's');
                    sigGen(signal1cos, 200, BUFLEN, 'c');
                    set_excitation_frequency(200);  // frequency in kHz
                    mux_counter = 0;
                    break;
                case 'D': // 400 kHz
                    sigGen(signal1sin, 400, BUFLEN, 's');
                    sigGen(signal1cos, 400, BUFLEN, 'c');
                    set_excitation_frequency(400);  // frequency in kHz
                    mux_counter = 0;
                    break;
                ...
                case 'B': // Sense B gain -
                    updateShiftRegister(0x00001C00, mirror(mirror(sr_current_val) - 0x00080000));
                    break;
                case 'r': // 2 Channel mode. Set mux for first 2 channels and don't change dynamically
                    num_z_channels = 2;
                    mux_counter = 0;
                    mux_busy_flag = 1;
                    router_config(num_pairs_1, mux_inputs_1, mux_outputs_1);
