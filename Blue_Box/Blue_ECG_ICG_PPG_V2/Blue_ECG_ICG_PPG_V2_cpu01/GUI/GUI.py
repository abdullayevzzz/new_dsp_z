...
def auto_find_port():
    ...
    # print([(port.device, port.description) for port in ports])
    # First check for wired connection
    for port in ports:
        if 'XDS100' in port.description:
            print(f"Wired connection found and connected on {port.device}")
            return port.device
    # If wired connection not found, try to find the bluetooth device by sending and expecting bytes
    for port in ports:
        print(f"Trying {port.device}...")
        try:
            with serial.Serial(port.device, 460800, timeout=0.5) as temp_ser:
                temp_ser.write(b'\x01')
                time.sleep(0.2)
                temp_ser.reset_input_buffer()
                response = temp_ser.read(1)
                # print(hex(response[0]))
                if response == b'\x02':
                    print(f"Bluetooh connection found and connected on {port.device}")
                    return port.device
