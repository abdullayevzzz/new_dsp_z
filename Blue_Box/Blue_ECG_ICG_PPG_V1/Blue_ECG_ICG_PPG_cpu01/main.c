...
uint16_t mux_period = 1000; // mux period in ms
    ...
    while(1){
        ...
        if (halfFilled) //wait until half buffer to fill
        {
            ...
            if (!first_time){
                int j;
                for (j=12; j<24; j++){
                    HWREGH(SCIA_BASE + SCI_O_TXBUF) = data[j];  // UART
                    HWREGH(SPIA_BASE + SPI_O_TXBUF) = data[j];  // SPI
                 }
            }
            first_time = 0;
            halfFilled = 0;
