...
while True:
    ...
    if len(buffer) != packet_size:
        ...
        if tomo_mode is True:
            ...
            if mux_mode_cur & 0x40000000:
                tomo_sense_updated = True
                # print("sense updated")
                if mux_mode_cur & 0x80000000:
                    tomo_exc_updated = True
                # print("exc updated")
                if Z1_TOMO_MAG:
                    Z1_TOMO_MAG_AVG = sum(Z1_TOMO_MAG) / len(Z1_TOMO_MAG)
                if Z2_TOMO_MAG:
                    Z2_TOMO_MAG_AVG = sum(Z2_TOMO_MAG) / len(Z2_TOMO_MAG)
                Z1_TOMO_MAG = []
                Z1_TOMO_MAG = []
                Z1_TOMO_PHASE = []
                Z2_TOMO_PHASE = []
                continue  # skip this sample
            Z1_TOMO_MAG.append(magZ1)
            Z1_TOMO_PHASE.append(phZ1)
            Z2_TOMO_MAG.append(magZ2)
            Z2_TOMO_PHASE.append(phZ2)
    ...
    if logFlag:
        if tomo_mode is False:
        ...
        elif (tomo_mode is True) and (tomo_sense_updated is True):  # Only log 1 averaged value per mux position
            ...
            writer.writerow([round(sum(Z1_TOMO_MAG) / len(Z1_TOMO_MAG), 6), round(sum(Z1_TOMO_PHASE) / len(Z1_TOMO_PHASE), 6),
    ...
    if (circular_counter >= 300) and tomo_mode is False:
    ...
    elif tomo_mode is True and tomo_sense_updated is True:
        tomo_sense_updated = False
        ...
        if tomo_exc_updated:
            ...
            # axs[exc_pin_index - 1 if exc_pin_index else 15].figure.canvas.draw_idle()
            # fig.canvas.draw_idle()
