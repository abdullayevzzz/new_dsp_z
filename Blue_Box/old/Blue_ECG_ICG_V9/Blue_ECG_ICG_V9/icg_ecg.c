...
int mux_inputs_1[] = {EXC_R, RET_GND, SNS_AP, SNS_AN}; //EXC_I 0, EXC_R, RET_TIA, RET_GND, SNS_AP, SNS_AN, SNS_BP, SNS_BN
int mux_outputs_1[] = {OUT_1, OUT_2, OUT_3, OUT_4}; // Outputs are in range 1-32
int num_pairs_1 = 0; // Enter the number of pairs

int mux_inputs_2[] = {EXC_R, RET_GND, SNS_AP, SNS_AN};
int mux_outputs_2[] = {OUT_7, OUT_8, OUT_9, OUT_10}; // Outputs are in range 1-32
int num_pairs_2 = 4; // Enter the number of pairs

//#include <sine_wave_gen.c>
void sigGen (int16_t signal[], int f, int len, char s); // f in kHz, s = 's' for sine, 'c' for cosine
void pack (uint8_t* data, volatile uint16_t *packetNumber, int32_t  *accumA1I, int32_t  *accumA1Q, int32_t  *accumB1I, int32_t  *accumB1Q, int32_t  *accumC1I, int32_t *accumC1Q, volatile uint32_t *accumD,  char *mode);
...
// Globals
const uint32_t sr_initial_val = 0b00010010001001111111111111111100;
//................................ISsIIIA_AABBBEEEMMMMMMMMMMMMMMMM
uint32_t sr_current_val = sr_initial_val;
uint16_t adcAResult = 0;
uint16_t adcBResult = 0;
uint16_t adcCResult = 0;
uint32_t adcDResult = 0;
volatile uint32_t  accumD = 0;  //accumulate ECG data

uint16_t adcAResults[BUFLEN] = {0};
uint16_t adcBResults[BUFLEN] = {0};
uint16_t adcCResults[BUFLEN] = {0};
...
//--    Board_init();
    ...
    router(0, mux_inputs_1, mux_outputs_1);
    ...
    while(1){
        ...
        */
        ...
        if (fullFilled == 1) //wait until full buffer to fill
        {
            ...
            //send result
            pack (data, &packetNumber, &accumA1I, &accumA1Q, &accumB1I, &accumB1Q, &accumC1I, &accumC1Q, &accumD, &mode);
...
void initEPWM_ADC(void)
{
    ...
    // 250kHz sampling rate is obtained by these values
    EPWM_setCounterCompareValue(EPWM_FOR_ADC_BASE , EPWM_COUNTER_COMPARE_B, 200);
    EPWM_setTimeBasePeriod(EPWM_FOR_ADC_BASE , 399);
...
__interrupt void adcA1ISR(void)
{
    ...
    adcDResult += ADC_readResult(ADCDRESULT_BASE, ADC_SOC_NUMBER2);


    //uint16_t test = ADC_readResult(ADCDRESULT_BASE, ADC_SOC_NUMBER2);
    //
   ...
   if (PCB==0) {
       ...
       adcCResults[counter]=(adcCResult>>1Ul); //
       // adcDResults[counter]=adcDResult; //
       counter++;

       /*
       adcAResults[counter]=adcAResult; //
       adcBResults[counter]=adcBResult; //
       adcCResults[counter]=adcCResult; //
        */
   }
    ...
    if (counter > BUFLEN - 1){
        ...
        counter = 0;
        packetNumber++;
        accumD = adcDResult>>0UL; //sum of 500 samples - divided by 1
        adcDResult = 0;
    }
