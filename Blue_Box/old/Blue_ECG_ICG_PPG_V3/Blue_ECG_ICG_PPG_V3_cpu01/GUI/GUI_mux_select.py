...
mux_time_ms = 1000
packet_size = 24
num_z_channels = 4  # default number of BIOZ channels
...
def create_buttons():
    ...
    button_axs.append(plt.axes([0.70, 0.02, 0.1, 0.1]))
    ...
    # Range buttons
    button_axs.append(plt.axes([0.70, 0.22, 0.05, 0.03]))
    buttons.append(Button(button_axs[-1], '+', color="yellow"))
    buttons[-1].on_clicked(signal_ranges.next)

    button_axs.append(plt.axes([0.70, 0.18, 0.05, 0.04], frameon=True))
    ...
    button_axs.append(plt.axes([0.70, 0.15, 0.05, 0.03]))
    buttons.append(Button(button_axs[-1], '-', color="yellow"))
    buttons[-1].on_clicked(signal_ranges.prev)

    button_axs.append(plt.axes([0.75, 0.15, 0.05, 0.1], facecolor='white'))
...
def try_connect_bluetooh(port_device, result):
    try:
...
def auto_find_port():
    ...
    for port in ports:
        ...
        connect_thread = threading.Thread(target=try_connect_bluetooh, args=(port.device, result))
