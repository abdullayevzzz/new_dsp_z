...
while not keyboard.is_pressed("s"):
    ...
    if (buffer[0] == 0x7F and buffer[1] == 0xFF):   #confirm synchronization
        ...
        if (j==(halfLen-1)):
            axA.cla()
            #axA.plot(mags_aver[halfLen//5:(fulLen//5+1)] + mags_aver[0:(halfLen//5)])
    ...
    i = 0
    while not (buffer[i] == 0x7F and buffer[(i+1, 0)[i+1 == 12]] == 0xFF): #modify code, make unique sync. code
        i = i+1    #find syncronization
    ser.read(i) #throw away until next syncronisation
