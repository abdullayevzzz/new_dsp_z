    ...
    if (buffer[0] == 0x7F and buffer[1] == 0xFF):   #confirm synchronization
        if mode == 'd':
            ...
            if (j==fulLen):
                ...
                if (filter_50_mode):
                    ...
                    for m in range ((fulLen)-4):
                        ECG_filt[m]  = (ECG_filt[m] + ECG_filt[m+1] + ECG_filt[m+2] + ECG_filt[m+3])/4
                        ECG_filt[fulLen-4:fulLen-1] = ECG_filt[fulLen-4:fulLen-1] #end case
                #if (filter_100_mode):
                    #ECG_aver = signal.filtfilt(b100, a100, ECG_aver) #100Hz remove
                    #m = 0
                    #for m in range ((fulLen//5)-2):
                    #    ECG_filt[m]  = (ECG_filt[m] + ECG_filt[m+1])/2
                    #    ECG_filt[(fulLen//5)-2:(fulLen//5)-1] = ECG_filt[(fulLen//5)-2:(fulLen//5)-1] #end case
                
                if (logFlag):
                    for r in range (fulLen):
                        data2 = ["%.4f" % mags[r],"%.4f" % ECG[r], "%.4f" % ECG_filt[r]]
            ...
            if (j==fulLen):
                ...
                if (logFlag):
                    for r in range (fulLen):
                        data4 = ["%.4f" % mags[r],"%.4f" % ECG[r], "%.4f" % ECG_filt[r]]
