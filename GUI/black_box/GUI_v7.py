...
ax8 = plt.axes([0.55, 0.02, 0.1, 0.1])
b6 = Button(ax8, 'Stop',color="yellow")
b6.on_clicked(stop)
    ...
    if (buffer[0] == 0x7F and buffer[1] == 0xFF):   #confirm synchronization
        if mode == 'd':
            ...
            if (j>0 and j%5 == 4):
                ...
                data2 = ["%.4f" % mags_aver[k+1-fulLen//5],"%.4f" % ECG_aver[k+1-fulLen//5]] #previous value which is filtered
            ...
            if (j>0 and j%5 == 4):
                ...
                #k = k - 1
                #print(ECG_aver[k])
                data4 = ["%.4f" % mags_aver[k+1-fulLen//5],"%.4f" % ECG_aver[k+1-fulLen//5]]
        ...
        # if (j==(halfLen-1)):
            ...
            # axB.cla()
            # axB.plot(diffmags_aver[int(halfLen//5):int(fulLen//5)] + diffmags_aver[0:int(halfLen//5)])
            # axB.set_title('diff ICG', fontweight ="bold")

            # axC.cla()
            # axC.plot(ECG_aver[int(halfLen//5):int(fulLen//5)] + ECG_aver[0:int(halfLen//5)])
