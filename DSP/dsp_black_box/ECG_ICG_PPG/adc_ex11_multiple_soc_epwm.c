...
#define EPWM_TIMER_TBPRD2    6250UL
...
void pack (uint8_t* data, volatile uint16_t *packetNumber, int32_t  *accumA1I, int32_t  *accumA1Q, int32_t  *accumB1I, int32_t  *accumB1Q, volatile uint32_t *accumC, uint32_t *ppg_result,  char *mode);
...
{   sigGen(signal1sin,10,BUFLEN, 's');
    ...
    // Initialize GPIOs 32 (104) and 33 (105) for use as SDA A and SCL A respectively
...
//--    Board_init();
    ...
    while(1){
        ...
        if (halfFilled == 1) //wait until half buffer to fill
        {
           ...
           //ppg data collection
           // happens at 200Hz rate. PPG samples at 400Hz rate. 1-3 new samples expected
            uint32_t ppg_result_temp = 0;
            ppg_result = 0;
            int8_t m;
            int8_t dif = numOfNewSamples();
            for (m = dif; m>0; m--){
               readRegister(MAX30105_FIFODATA, ppg_sample, 3);
               ppg_result_temp = (ppg_sample[0] << 6) | (ppg_sample[1] >> 2); //16 bit ADC in 3 bytes, 17th bit is MSB
               ppg_result_temp = (ppg_result_temp << 8) | ((ppg_sample[1] << 6) | (ppg_sample[2] >> 2));
               ppg_result = ppg_result + ppg_result_temp;
             }
            if(dif!=0)
                ppg_result = ppg_result / dif;
            //end of ppg data collection
        ...
        if (fullFilled == 1) //wait until full buffer to fill
        {
            ...
            //send result
            pack (data, &packetNumber, &accumA1I, &accumA1Q, &accumB1I, &accumB1Q, &accumC, &ppg_result, &mode);
        ...
        if(SCI_getRxFIFOStatus(SCIA_BASE) != SCI_FIFO_RX0){
            ...
            switch (r){
                ...
                case '1': //200Hz
                    ...
                    EPWM_setClockPrescaler(myEPWMk_BASE, EPWM_CLOCK_DIVIDER_2, EPWM_HSCLOCK_DIVIDER_1);
                ...
                case '3': //20kHz
                    ...
                    EPWM_setClockPrescaler(myEPWMk_BASE, EPWM_CLOCK_DIVIDER_1, EPWM_HSCLOCK_DIVIDER_1);
                    EPWM_setTimeBasePeriod(myEPWMk_BASE, EPWM_TIMER_TBPRD2/5);
                    EPWM_setCounterCompareValue(myEPWMk_BASE, EPWM_COUNTER_COMPARE_A, EPWM_TIMER_TBPRD2/10);
                    break;
            }
        }
