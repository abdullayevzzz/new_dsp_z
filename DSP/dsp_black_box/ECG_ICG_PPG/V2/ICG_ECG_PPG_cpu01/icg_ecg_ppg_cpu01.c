...
// Globals
uint16_t cpu1RArray[256];       // Mapped to GS14 of shared RAM owned by CPU2
#pragma DATA_SECTION(cpu1RArray,"SHARERAMGS14");
...
void main(void)
{
    ...
    //Give memory access to GS14 to CPU2
    ...
    while(1){
        ...
        if (halfFilled == 1) //wait until half buffer to fill
        {
            ...
            //PPG data from the second Core
            ppg_data = cpu1RArray[0];
        ...
        if (fullFilled == 1) //wait until full buffer to fill
        {
            ...
            //send result
            pack (data, &packetNumber, &accumA1I, &accumA1Q, &accumB1I, &accumB1Q, &accumC, &ppg_data, &mode);
