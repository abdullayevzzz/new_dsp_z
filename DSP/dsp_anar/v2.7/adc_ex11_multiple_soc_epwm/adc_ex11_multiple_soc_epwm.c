...
#define  PCB  0 // 0 for Anar's board, 1 for Marek's board
...
void pack (uint8_t* data, volatile uint16_t *packetNumber, int32_t  *accumA1I, int32_t  *accumA1Q, int32_t  *accumB1I, int32_t  *accumB1Q);
...
/*
        ...
        if (test){  //calculate just once
            sinGenX (syntWaveA, BUFLEN, 25000, 0);
            sinGenX (syntWaveB, BUFLEN, 25000, 90);
            int i;

            accumA1I = _dmac(signal1sin,syntWaveA,BUFLEN/10-1,1);
            accumA1Q = _dmac(signal1cos,syntWaveA,BUFLEN/10-1,1);
            accumB1I = _dmac(signal1sin,syntWaveB,BUFLEN/10-1,1);
            accumB1Q = _dmac(signal1cos,syntWaveB,BUFLEN/10-1,1);
...
//    adcDResult1 = ADC_readResult(ADCDRESULT_BASE, ADC_SOC_NUMBER1);
 ...
 //   static uint16_t excitIndex = 0;
   if (PCB==0) {
       ...
       //adcAResults[counter]=adcAResults[counter]>>4;
       adcBResults[counter]=adcBResult-32767;
       //adcBResults[counter]=adcBResults[counter]>>4;
       counter++;
   }
...
#define EPWM_TIMER_TBPRD    1250UL
